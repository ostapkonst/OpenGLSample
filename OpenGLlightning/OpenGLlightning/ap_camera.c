...
void Camera_Setup(GLuint program, Camera * camera) {
	gl_mat4 view = m4_multipli(gr_rotation_v(camera->rotation), gr_translation_v(v3_mult_value(camera->position, -1)));

	glUniformMatrix4fv(glGetUniformLocation(program, "MVP"), 1, GL_TRUE, m4_multipli(camera->projection, view));
	glUniformMatrix4fv(glGetUniformLocation(program, "M"), 1, GL_TRUE, gr_translation_v(v3_mult_value(camera->position, -1)));
	glUniformMatrix4fv(glGetUniformLocation(program, "V"), 1, GL_TRUE, gr_rotation_v(camera->rotation));
}