...
void Camera_Setup(GLuint program, Camera * camera) {
	glUniformMatrix4fv(glGetUniformLocation(program, "MVP"), 1, GL_TRUE, m4_multipli(camera->projection, m4_multipli(gr_rotation_v(camera->rotation), gr_translation_v(v3_mult_value(camera->position, -1)))));
}